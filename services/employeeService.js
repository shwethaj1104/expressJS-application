...
exports.getAllEmployees = async (req, res) => {
    ...
    try {
        if(myCache.has("ListOfData")){
            ...
            res.status(200).json({ message: 'Cache hit', employee, mykeys })
        }
        else{
            employee = await Employees.find()
            myCache.set("ListOfData",employee)
            res.status(200).json({ message: 'Cache miss' ,employee,mykeys})
        }
    } catch (err) {
...
exports.addEmployee = async (req, res) => {
    ...
    try {
        ...
        res.status(200).status(201).json(newSubscriber)
    } catch (err) {
...
exports.deleteAllFromCache = async (req, res) => {
    try {   
        ...
        let mykeys = myCache.keys();
        res.status(200).json({ message: 'Deleted cache', mykeys })
    } catch (err) {
        res.status(500).json({ message: err.message })
    }
}
